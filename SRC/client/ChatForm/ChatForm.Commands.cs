...
namespace RobloxChatLauncher
{
    public partial class ChatForm : Form
    {
        ...
        private async Task<bool> HandleCommands(string input)
        {
            ...
            switch (command)
            {
                case "/coregui": return HandleCoreGui(args);
                case "/cookie":
                    rawInputText = ""; SyncInput();
                    if (string.IsNullOrWhiteSpace(args)) { RichChatBox.AppendSystemMessage(chatBox,"Usage: /cookie <cookie value> to save locally, or /cookie clear. This command does not open the GUI; use /coregui 2013 or /coregui 2016."); return true; }
                    try { LocalRobloxAccount.Save(args.Trim()); RichChatBox.AppendSystemMessage(chatBox,args.Trim()=="clear" ? "Local cookie removed." : "Cookie encrypted locally. It is never sent to the chat server."); }
                    catch { RichChatBox.AppendSystemMessage(chatBox,"Cookie could not be saved. Use /cookie <value> or /cookie clear."); }
                    return true;
                /// <summary>Changes the chat server to a Radmin IPv4 address and reconnects.</summary>
                case "/radmin":
                    if (!Core.ServerEndpoint.TrySetRadminAddress(args, out string addressError))
                    {
                        RichChatBox.AppendSystemMessage(chatBox, addressError);
                        return true;
                    }
                    RichChatBox.AppendSystemMessage(chatBox, $"Server changed to {Core.ServerEndpoint.HttpBaseUrl}. Reconnecting...");
                ...
                case "/c":
                    chatBox.Clear();
                    RichChatBox.ClearLines(chatBox);
                    return true;
                ...
                case "/channel":
                    ...
                    {
                        var channels=new ChannelListForm(channelId);
                        channels.JoinRequested+=async id=>{ if(string.IsNullOrWhiteSpace(id)||id.Length>128)return;channelId=id;channels.Close();await RestartWebSocketAsync(); };
                        channels.Show(); return true;
                    }
                ...
                /// <param>args: One of strict, default, relaxed, or off. Empty args displays current setting.</param>
                case "/filter":
                    return HandleFilterPreferenceCommand(args);

                /// <summary>Changes the chat appearance to a classic Roblox era.</summary>
                case "/theme":
                    return HandleThemeCommand(args);

                case "/drag":
                    return HandleDragCommand(args);
                case "/text":
                ...
                case "/emote":
                    ...
                    return true;
                case "/e":
                    return await HandleRobloxEmote(args);
