...
const { db, initDatabase } = require('./db/sqlitePool');
...
if (!Env.PERSPECTIVE_API_KEY) {
    console.warn('[NOTICE] PERSPECTIVE_API_KEY is not set; using the built-in local safety policy.');
}
...
function broadcastRoster(channelId) {
    ...
    const members = [...(channels.get(channelId) || [])]
        ...
        .map(ws => ({ id: ws.memberId, name: ws.senderName, robloxId: ws.robloxId || null,
            sessionSeconds: Math.max(0, Math.floor((now - ws.sessionStartedAt) / 1000)),
            totalSeconds: (ws.previousTotalSeconds || 0) + Math.max(0, Math.floor((now - ws.sessionStartedAt) / 1000)),
            timezoneOffsetMinutes: ws.timezoneOffsetMinutes,
            timezoneLabel: ws.timezoneLabel }));
    broadcastToChannel(channelId, { type: 'roster', channelId, members });
}

app.get('/api/v1/channels', (req, res) => {
    const data = [...channels.entries()].map(([id, clients]) => ({
        id, users: [...clients].filter(ws => ws.readyState === WebSocket.OPEN).length
...
function enqueueMessage(text, allowProfanity = false) {
    return new Promise((resolve) => {
        ...
        messageQueue.push({ text, allowProfanity, resolve });
        processQueue();
    });
}
...
async function processQueue() {
    ...
    processing = true;
    const { text, allowProfanity, resolve } = messageQueue.shift();

    try {
        const result = await isMessageAllowed(text, { allowProfanity });
        if (!result.allowed) {
            resolve({
                allowed: false,
...
app.get('/api/v1/admin/verified', validateRead, async (req, res) => {
    try {
        const rows = db.prepare('SELECT hwid, roblox_id FROM verified_users ORDER BY roblox_id').all();
        res.json(rows); // Returns an array of objects { hwid, roblox_id }
    } catch (err) {
...
app.get(
    ...
    async (req, res) => {
        try {
            const game = db.prepare(`SELECT universe_id, is_unlisted, created_at
                 FROM game_registry
                 WHERE universe_id = ?
                 LIMIT 1`).get(Number(req.universeId));
            ...
            res.json({
                ...
                isUnlisted: !!game.is_unlisted, // SQLite stores booleans as 0 or 1
...
app.patch(
    ...
    async (req, res) => {
        ...
        try {
            db.prepare('UPDATE game_registry SET is_unlisted = ? WHERE universe_id = ?')
              .run(isUnlisted ? 1 : 0, Number(req.universeId));
...
setInterval(() => channels.forEach((_, channelId) => broadcastRoster(channelId)), 5000).unref();

setInterval(() => {
    const now = Date.now();
    channels.forEach(clients => clients.forEach(ws => {
        if (!ws.usageKey || !ws.sessionStartedAt || ws.readyState !== WebSocket.OPEN) return;
        const elapsed = Math.max(0, Math.floor((now - ws.sessionStartedAt) / 1000));
        if (elapsed < 1) return;
        ws.sessionStartedAt = now;
        ws.previousTotalSeconds = (ws.previousTotalSeconds || 0) + elapsed;
        try {
            db.prepare(`INSERT INTO chat_usage(user_key,total_seconds,updated_at) VALUES(?, ?, datetime('now'))
                ON CONFLICT(user_key) DO UPDATE SET 
                    total_seconds = chat_usage.total_seconds + EXCLUDED.total_seconds,
                    updated_at = datetime('now')`)
              .run(ws.usageKey, elapsed);
        } catch (err) {
            console.error('Usage checkpoint failed:', err.message);
        }
    }));
}, 30000).unref();
...
wss.on('connection', (ws, req) => {
    ...
    const trustedIp = getTrustedIp(req, ws);
    const connectionPort = trustedIp.split(':')[1] || '0';
    ...
    ws.on('message', async (data) => {
        try {
            ...
            if (payload.type === 'join') {
                const { channelId, hwid, usageId } = payload;
                if (typeof channelId !== 'string' || !channelId.length || channelId.length > 128) return;
                ...
                ws.robloxId = robloxId ? String(robloxId) : null;
                ws.timezoneOffsetMinutes = Number.isInteger(payload.timezoneOffsetMinutes) && payload.timezoneOffsetMinutes >= -840 && payload.timezoneOffsetMinutes <= 840
                    ? payload.timezoneOffsetMinutes : null;
                ws.timezoneLabel = typeof payload.timezoneLabel === 'string' && /^[A-Za-z0-9+\-]{1,8}$/.test(payload.timezoneLabel)
                    ? payload.timezoneLabel.toUpperCase() : null;
                    
                if (ws.usageKey && ws.sessionStartedAt) {
                    const elapsed = Math.max(0, Math.floor((Date.now() - ws.sessionStartedAt) / 1000));
                    try {
                        db.prepare(`INSERT INTO chat_usage(user_key,total_seconds,updated_at) VALUES(?, ?, datetime('now'))
                            ON CONFLICT(user_key) DO UPDATE SET 
                                total_seconds = chat_usage.total_seconds + EXCLUDED.total_seconds,
                        ...
                        console.error('Usage persist failed:', err.message);
                    }
                }
                
                const safeUsageId = typeof usageId === 'string' && /^[a-f0-9]{32}$/i.test(usageId) ? usageId.toLowerCase() : userKey;
                ws.usageKey = ws.robloxId ? `roblox:${ws.robloxId}` : `guest:${safeUsageId}`;
                ws.sessionStartedAt = Date.now();
                
                try {
                    const row = db.prepare('SELECT total_seconds FROM chat_usage WHERE user_key = ?').get(ws.usageKey);
                    ws.previousTotalSeconds = Number(row?.total_seconds || 0);
                } catch {
                    ws.previousTotalSeconds = 0;
                }
                ...
                if (previousChannel && previousChannel !== channelId) {
                    ...
                    if (prevClients) {
                        ...
                        broadcastRoster(previousChannel);
                ...
                ws.send(JSON.stringify({ type: 'identity', memberId: ws.memberId }));
                broadcastRoster(channelId);
            }

            if (payload.type === 'message' && currentChannel) {
                const moderation = await enqueueMessage(
                    payload.text,
                    payload.filterPreference === 'off'
                );
    ...
    ws.on('close', () => {
        ...
        if (ws.usageKey && ws.sessionStartedAt) {
            const seconds = Math.max(0, Math.floor((Date.now() - ws.sessionStartedAt) / 1000));
            try {
                db.prepare(`INSERT INTO chat_usage(user_key,total_seconds,updated_at) VALUES(?, ?, datetime('now'))
                    ON CONFLICT(user_key) DO UPDATE SET 
                        total_seconds = chat_usage.total_seconds + EXCLUDED.total_seconds, 
                        updated_at = datetime('now')`)
                  .run(ws.usageKey, seconds);
            } catch (err) {
